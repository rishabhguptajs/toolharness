...
This is the opt-in path behind ``evalharness live``. It is deliberately *safe by
...
def prepare_workdir(repo: Path, *, sandbox: bool = True) -> tuple[Path, bool]:
    ...
    if not sandbox:
        return repo, False
    tmp = Path(tempfile.mkdtemp(prefix="evalharness-live-"))
    dest = tmp / repo.name
    ...
    try:
        ...
        if trace_path is not None:
            ...
            fd = tempfile.NamedTemporaryFile(
                prefix=f"evalharness-{prof.name}-", suffix=".trace", delete=False
            )
            trace = Path(fd.name)
            fd.close()
