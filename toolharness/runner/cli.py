"""`toolharness` command-line entry point.

    toolharness run <trace.json> [--adapter N] [--task spec.yaml] [--judge P]
                    [--json OUT] [--html OUT] [--fail-under N]
                    [--fail-under-mode MODE=N ...]
    toolharness live --adapter N --task spec.yaml [--repo DIR] [--in-place]
                    [--timeout S] [--json OUT] [--html OUT] [--fail-under N]
    toolharness compare <trace.json> ... --html OUT [--task spec.yaml] [--judge P]
...
from toolharness.detectors.judge import JudgeError, build_judge
from toolharness.report.html_report import write_html_dashboard, write_html_report
from toolharness.report.json_report import report_dict, write_json_report
...
def _cmd_live(args: argparse.Namespace) -> int:
    from toolharness.runner.live import run_live
...
def _cmd_suite(args: argparse.Namespace) -> int:
    from toolharness.report.html_report import write_html_dashboard
...
def _cmd_benchmark(args: argparse.Namespace) -> int:
    from toolharness.eval.benchmark import bfcl_report, format_report
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="toolharness")
